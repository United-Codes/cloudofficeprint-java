...
/**
 * A collection used to group multiple RenderElements together. It can contain
 * nested `Object`s and should be used to pass multiple `RenderElements` as
 * PrintJob data, as well as to allow for nested elements. Its name is used as a
 * key name when nested, but ignored for all purposes when it's the outer
 * object.
 */
public class ElementCollection extends RenderElement {
        ...
        for (Map.Entry<String, JsonElement> entry : json.entrySet()) {
            if (entry.getValue() instanceof JsonObject) {
                ...
                if (entry.getValue().toString().equals("\"null\"")) {
                    ...
                    Property property = new Property(entry.getKey().toString(),
     ...
     * @param json2 Json to take the data from. Cannot have nested
     *              JSON's/JsonArrays.
     */
